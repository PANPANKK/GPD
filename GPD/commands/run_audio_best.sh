...
OUT_DIR="${OUT_DIR:-$OUT_ROOT/audio_best}"
mkdir -p "$OUT_DIR"

DATA_ROOT="${AUDIO_DATA_ROOT:-/mnt/**/audio}"
SPLIT_ROOT="${AUDIO_SPLIT_ROOT:-/mnt/**/data_split/5fold}"
TEACHER_REPO_ROOT="${AUDIO_TEACHER_REPO_ROOT:-/mnt/**/gsr_time2graphplus}"
TEACHER_CKPT_ROOT="${AUDIO_TEACHER_CKPT_ROOT:-/mnt/**/results/kd_staged/xkd_exp01/stage1_teacher}"
...
"$PYTHON_BIN" "$TRAIN_SCRIPT" \
  ...
  --nofeat_route_digit_enter 0.78 --nofeat_route_digit_exit 0.86 \
  --feat_gate_mode off \
...
echo "[DONE] audio summary: $OUT_DIR/summary.json"
